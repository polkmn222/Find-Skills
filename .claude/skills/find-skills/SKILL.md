---
name: find-skills
description: Use in Claude Code when the user asks to find relevant project skills for a project or idea. Runs a checklist-first workflow, ranks local and external skill candidates, writes a project-local search index, and gates installation or implementation.
---
...
- Search current-project skills first from `.claude/skills/**/SKILL.md`.
...
- Install only: install or copy only the requested/indexed skills into the current project's `.claude/skills/` directory; do not implement, scaffold, or refactor.
...
6. Search project-local skills in `.claude/skills/**/SKILL.md`.
7. Search external Git/community/official sources unless the checklist forbids or limits external discovery.
8. Check required bootstrap skills in the current project's `.claude/skills/` directory:
   - `skill-installer`
   - `skill-creator`
   - `agents-md`
   - `agent-orchestrator`
...
13. If installation is requested, install/copy required skills into project-local `.claude/skills/`, verify each `SKILL.md`, and update the index installation status.
...
Global or home-level installs such as `~/.claude/skills` or `~/.agents/skills` do not satisfy project-local installation. Use them only as sources, caches, or temporary installs. See `references/project-local-installation.md`.
