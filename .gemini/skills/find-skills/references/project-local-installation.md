...
These paths may be used only as sources, caches, or temporary global installs. If a skill is installed globally first, copy or sync it into the current project's `.gemini/skills/` directory before marking it installed.
...
```bash
test -f .gemini/skills/<skill-name>/SKILL.md
```
